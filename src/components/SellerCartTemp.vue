...
export default {
  ...
  methods: {
    ...
    openEdit () {
      if (!this.isEdit) this.logEvent({ event: 'click_app_quotation_modify', type: '01' })
      this.pickWayShow = false
    ...
    // 清空购物车
    clearCarts () {
      this.logEvent({ event: 'click_app_quotation_del_all', type: '01' })
      const self = this
    ...
    choosedAll () {
      this.logEvent({ event: 'click_app_quotation_checkall', type: '01' })
    ...
    auditDxCheck (flag) {
      ...
      setTimeout(async () => {
        this.hideLoading()
        try {
          if (flag === 2) this.logEvent({ event: 'click_app_quotation_generate', type: '01' })
          let filterArray = this.carts.filter(itm => itm.choosed === true)
          // 编辑操作
          if (this.isEdit) {
            if (flag === 1) this.logEvent({ event: 'click_app_quotation_del', type: '01' })
            this.btnDisable = false
            if (filterArray.length === 0) {
            ...
            this.logEvent({ event: 'click_app_quotation_dx', type: '01' })
          }
          // 定向操作
          if (filterArray.length === 0) {
