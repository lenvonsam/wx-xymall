                ...
                .content.ft-13
                  .flex.flex-center.justify-between
                    div
                      ...
                      .pt-5
                        span {{cart.ratioAvailableAmount}}支 /
                          //-  {{cart.cartQuantityType == '02' ? cart.ratioAvailablePoundWeight : cart.ratioAvailableManagerWeight}}
                        span(v-if="cart.onlineQuantityType === '01'") {{cart.ratioAvailableManagerWeight}}
                        span(v-else-if="cart.onlineQuantityType === '02'") {{cart.ratioAvailablePoundWeight}}
                        span(v-else) {{cart.ratioAvailableManagerWeight}}
                        span 吨
      ...
      if (flag.toString() === 'confirm') {
        ...
        self.httpPost(this.apiList.zf.generateContract, { cartItem: filterArray, deliveryType: '01', sourceType: '03' }).then(res => {
          ...
          if (salesman) {
            ...
            let content = `您的客户${this.currentUser.companyName}，${time}生成物资销售合同【移动】，合同编号${res.data.saleContractNo}，请及时进行确认，联系电话${this.currentUser.phone}`
