      ...
      .pt-half-rem(v-if="billDetail.status_desc !== '待补款' && billDetail.status_desc !== '违约'")
        .bg-white
          ...
          .ft-13.padding-sm.text-gray
            ...
            .flex.padding-bottom-xs
              .col.text-right
                span.ft-12.pr-5(v-if="billDetail.payMethod === '02'") (现金支付：{{billDetail.cashPayMoney}}元 + 白条支付：{{billDetail.creditUseMoney}}元)
...
export default {
  ...
  methods: {
    ...
    payBill () {
      this.logEvent({ event: 'click_app_myorder_detail_pay', type: '01' })
    ...
    cancelBill () {
      this.logEvent({ event: 'click_app_myorder_detail_cancel', type: '01' })
      const me = this
      if (!this.btnDisable) {
