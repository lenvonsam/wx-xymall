  ...
  .padding.bg-white(style="padding-top: 50px")
    ...
    swiper(v-if="gallery.length > 0", :indicator-dots="true", :autoplay="true", circular,indicator-active-color="#fff", indicator-color="rgba(255, 255, 255, .3)")
      swiper-item.border-radius(v-for="(g,idx) in gallery", :key="idx")
        img.response(:src="zfBASICURL + 'foundation/public/user/picture/view?path=' + g.bannerName", v-if="g.bannerName", style="height: 300rpx", mode="widthFix")
...
export default {
  ...
  methods: {
    ...
    // 跳转搜索页面
    jumpSearch () {
      this.currentUser.type === 'seller' ? this.logEvent({ event: 'click_app_index_search_seller', type: '01' }) : this.logEvent({ event: 'click_app_index_search', type: '01' })
    ...
    async loadBanner () {
      try {
        ...
        console.log('onlineBanner', this.gallery)
      } catch (e) {
        this.hideLoading()
