  ...
  .padding-sm
    div
      .bg-white.card
        ...
        .row
          span 低卖原因：
          span.text-bold {{detailData.sellLowReason}}
...
export default {
  ...
  methods: {
    ...
    async loadData () {
      ...
      console.log('低卖参数整理====>', this.detailData)
      ...
      this.detailData.list = listData.map(item => {
        return {
          ...
          diffPrice: Number(item.salePrice) - Number(item.inTaxPrice),
          oldSalePrice: item.inTaxPrice,
