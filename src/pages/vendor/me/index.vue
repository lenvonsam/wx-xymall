...
export default {
  ...
  methods: {
    ...
    jumpProfile () {
      this.logEvent({ event: 'click_app_me_information', type: '01' })
    ...
    jumpBillMore () {
      this.logEvent({ event: 'click_app_me_myorder_more', type: '01' })
      this.jump('/pages/bill/main')
    },
    jumpBicon (url) {
      // if (url.path === '/bill?tabName=1') this.logEvent({ event: 'click_app_me_to_pay_order' })
      // if (url.path === '/ladbill/confirm/list') this.logEvent({ event: 'click_app_me_to_confirm' })
      // if (url.path === '/ladbill?tabName=4') this.logEvent({ event: 'click_app_me_to_pay_contract' })
      // if (url.path === '/invoice') this.logEvent({ event: 'click_app_me_to_invoice' })
      this.jump(url.path)
    },
    serviceCallUrl () {
      this.logEvent({ event: 'click_app_me_tel', type: '01' })
